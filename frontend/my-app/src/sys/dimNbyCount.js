...
const dimNbyCount = (network, nodeGeo, edges, Count) => {
    ...
    const queue = network
    .filter(node => node.achievement_count>50*Count-50 && ( node.achievement_count<50*Count+1 || Count===21 )  ) // 筛选出 work_unit 为 Aca 的节点
    .map(node => node.id);     
    ...
    for (const edge of edges) {
        if(queue.includes(edge.source)||queue.includes(edge.target)){
            // console.log(1)
